...
//[db-inspect]  flagging ad this is where db uri is formatted
function formatDbUri(dbConf){
  var conf = config.getConfig();
  var replicaSet = (conf.mongo || {}).replicaSet;
  return buildConnectionString(dbConf, replicaSet);
}

//[db-inspect]  flagging as this is where the db connection string is built
function buildConnectionString(dbConf, replicaSet) {
  if (replicaSet) {
    return "mongodb://"+dbConf.user+":"+dbConf.pass + "@" + dbConf.host + ":" + dbConf.port+"/"+dbConf.name + "?replicaSet=" + replicaSet;
  } else {
    return "mongodb://"+dbConf.user+":"+dbConf.pass + "@" + dbConf.host + ":" + dbConf.port+"/"+dbConf.name ;
  }
}
