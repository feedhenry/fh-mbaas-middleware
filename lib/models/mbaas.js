...
var Schema = mongoose.Schema;
  ...
  MbaasSchema.statics.createModel = function(domain, env, config, cb){
    ...
    var userDbConf = getDomainDbConf(domain, env, config, true);
    var fields = {
      domain: domain,
      environment: env,
      dbConf: dbConf
...
function getDomainDbConf(domain, env, config, isUserDb){
  var db_name = domain + '_' + env;
  var mongo = isUserDb ? config.mongo_userdb : config.mongo;
